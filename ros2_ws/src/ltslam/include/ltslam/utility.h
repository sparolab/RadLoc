...
// --- de-ROS'd: standalone LT-SLAM (file-based in/out, no ROS) ---
#include <iostream>
// logging shim replacing ROS_INFO* macros
#define ROS_INFO_STREAM(x)  do { std::cout << x << std::endl; } while(0)
#define ROS_INFO(...)       do { printf(__VA_ARGS__); printf("\n"); } while(0)
#define ROS_WARN_STREAM(x)  do { std::cout << x << std::endl; } while(0)
#define ROS_WARN(...)       do { printf(__VA_ARGS__); printf("\n"); } while(0)
