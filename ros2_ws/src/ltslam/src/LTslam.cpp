...
void LTslam::run( void )
{
    ...
    // write accepted inter-session loop pairs (central_node query_node) for viz
    std::ofstream lf(save_directory_ + "inter_session_loops.txt");
    for (auto& p : accepted_inter_loops_) lf << p.first << " " << p.second << "\n";
    lf.close();
    std::cout << "wrote " << accepted_inter_loops_.size() << " inter-session loops to "
              << save_directory_ << "inter_session_loops.txt" << std::endl;
}
...
{
    // PhaRaO-style: relative measurement by PHASE CORRELATION between the two
    // keyframes' radar feature clouds (rasterized to cartesian BEV). Replaces ICP.
    mtx.lock();
    pcl::PointCloud<PointType>::Ptr cureKeyframeCloud(new pcl::PointCloud<PointType>());
    pcl::PointCloud<PointType>::Ptr targetKeyframeCloud(new pcl::PointCloud<PointType>());
    source_sess.loopFindNearKeyframesLocalCoord(cureKeyframeCloud, loop_idx_source_session, 0);
    target_sess.loopFindNearKeyframesLocalCoord(targetKeyframeCloud, loop_idx_target_session, 0);
    mtx.unlock();

    radloc::Transform2D rel = radloc::registerScans(targetKeyframeCloud->points, cureKeyframeCloud->points);

    // phase-correlation response gate (higher = more confident). loopFitnessScoreThreshold
    // is reused as a (1 - response) budget so the yaml knob still applies.
    double min_response = 1.0 - loopFitnessScoreThreshold;   // e.g. fit 0.7 -> resp >= 0.3
    if (!rel.valid || rel.response < min_response) {
        mtx.lock();
        std::cout << "  [SC loop] phaseCorr response failed (" << rel.response
                  << " < " << min_response << "). Reject this SC loop." << std::endl;
        mtx.unlock();
        return std::nullopt;
    }
    mtx.lock();
    std::cout << "  [SC loop] phaseCorr response passed (" << rel.response
              << " >= " << min_response << "). Add this SC loop." << std::endl;
    mtx.unlock();

    gtsam::Pose3 poseFrom = Pose3(Rot3::RzRyRx(0.0, 0.0, rel.theta), Point3(rel.x, rel.y, 0.0));
...
void LTslam::detectInterSessionSCloops() // using the RadLoc descriptor
{
    ...
    auto& target_index = target_sess.placeIndex;
    auto& source_index = source_sess.placeIndex;
    for (int source_node_idx=0; source_node_idx < int(source_index.size()); source_node_idx++)
    {
        // The descriptor is azimuth-averaged and therefore rotation invariant,
        // so a bare nearest-neighbour lookup is enough; the relative pose comes
        // later from phase correlation in doICPVirtualRelative.
        int loop_idx_target_session =
            target_index.detectLoopClosureIDBetweenSession(source_index.descriptor(source_node_idx));
...
void LTslam::addSCloops()
{
    ...
    for (int ith = 0; ith < num_scloops_sampled; ith++) 
    {
        ...
        if(relative_pose_optional) {
            ...
            accepted_inter_loops_.emplace_back(loop_idx_target_session, loop_idx_source_session);
