...
double cen2019features(cv::Mat fft_data, int max_points, int min_range, Eigen::MatrixXd &targets) {
    ...
    cv::Mat horizontal_edges, vertical_edges;
    cv::filter2D(fft_data, horizontal_edges, CV_32F, sobel_x);
    cv::filter2D(fft_data, vertical_edges, CV_32F, sobel_y);

    // Compute gradient magnitude
    cv::Mat g;
    cv::magnitude(horizontal_edges, vertical_edges, g);
